...
function compilerOptionsFromTsConfig ( typescript ) {
	...
	var tsconfig = typescript.readConfigFile( findFile( cwd, 'tsconfig.json' ), function (path$$1) { return fs.readFileSync( path$$1, 'utf8' ); } );
...
function fix ( code, id ) {
	...
	while ( match = re.exec( cleanCode ) ) {
		...
		code = erase$1( code, match.index, match[ 0 ].indexOf( match[ 2 ] ) );
...
function erase$1 ( code, start, length ) {
